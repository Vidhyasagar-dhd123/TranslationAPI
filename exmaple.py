...
def initialize_model_and_tokenizer(ckpt_dir, quantization, attn_implementation):
    ...
    tokenizer = AutoTokenizer.from_pretrained(ckpt_dir, trust_remote_code=True, local_files_only=False)
    ...
    if qconfig is None and DEVICE.startswith("cuda"):
        model.half()

    # Recommended: disable config-level cache to avoid relying on past_key_values
...
def batch_translate(input_sentences, src_lang, tgt_lang, model, tokenizer, ip):
    translations = []
    # small generation config we will use (no beam search, no cache)
    ...
    for i in range(0, len(input_sentences), BATCH_SIZE):
        ...
        with torch.no_grad():
            # explicitly pass generation_config and disable use_cache to avoid past_key_values issues
            generated_tokens = model.generate(
                **inputs,
                generation_config=gen_config,
                use_cache=False,           # double safety
                num_beams=1,               # avoid beam-search bug
                max_length=256,
                early_stopping=True,
            )
...
# ---------------------------------------------------------------------------
en_sents = [
    "My name is Vidhyasagar and I live in Bangalore.",
    "I loved her all the night",
]
